...
///
/// 진입 시 캐시 전체 프로필을 로드한다.
/// 저장 시 allergies·medications만 교체하고 나머지 필드(conditions/
/// symptomFrequency/diagnosed/triggerFoods/customTriggers)는 base에서 그대로 보존한다.
/// 서버에 GET/PATCH 없으므로 POST /onboarding 전체 재제출 방식 사용.
///
...
class _AllergyMedEditScreenState extends ConsumerState<AllergyMedEditScreen> {
  ...
  /// 프로필 캐시가 로드되면 최초 1회 로컬 상태를 초기화한다.
  ///
  ...
  Future<void> _onSave() async {
    ...
    final base = ref.read(healthProfileControllerProvider).valueOrNull;
    ...
    if (base == null) {
      ...
      final confirmed = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: Text(
            '저장하면 증상·트리거 등 다른 건강 정보가 초기화될 수 있어요. 계속할까요?',
    ...
    try {
      final baseProfile = base ?? const HealthProfile();
      // 핵심 보존 메커니즘: allergies·medications만 교체, 나머지는 base에서 그대로 유지
      final next = baseProfile.copyWith(
        allergies: _selectedAllergies.toList(),
        medications: _medications,
      );
      await profileController.submit(next);

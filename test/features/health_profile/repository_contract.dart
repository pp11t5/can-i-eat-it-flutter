...
void healthProfileRepositoryContract(
  ...
  // ---------------------------------------------------------------------------
  group('updateHealthInfo — allergies 부분 갱신', () {
    test('updateHealthInfo 후 currentProfile의 allergies가 갱신된다', () async {
      final repo = create();
      await repo.submitProfile(HealthProfile.sampleGerd());
    ...
    test('updateHealthInfo는 conditions 등 다른 필드를 보존한다', () async {
      final repo = create();
      await repo.submitProfile(HealthProfile.sampleGerd());
      await repo.updateHealthInfo(allergies: ['wheat']);

      final profile = await repo.currentProfile();
      expect(
          profile!.conditions, equals(HealthProfile.sampleGerd().conditions));
      expect(
